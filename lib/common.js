...
module.exports = function (cfg, deps) {
  ...
  var dataserviceshost = config.dataservicesApi;
  ...
  /**
   * Set DataServices host
   *
   * @param newhost Sets the DataServices host to a new value
   */
  function setDataServicesHost(newhost) {
    dataserviceshost = newhost;
  }
  /**
   * Get the DataServices host
   *
   */
  function getDataServicesUrl() {
    if (usersToken == null) {
      return null;
    }
    return makeDataServicesUrl('', { token: usersToken });
  }
  /**
   * Set Blip host
   *
  ...
  function makeDataServicesUrl(path, query) {
    if (dataserviceshost == null) {
      return null;
    }

    var result = dataserviceshost + path;
    if (!_.isEmpty(query)) {
      result += '?';
      var fields = [];
      for (var k in query) {
  ...
  return {
    ...
    makeDataServicesUrl : makeDataServicesUrl,
    makeBlipUrl : makeBlipUrl,
    setBlipHost :  setBlipHost,
    setUploadHost : setUploadHost,
    setDataServicesHost : setDataServicesHost,
    setApiHost : setApiHost,
    getUploadUrl : getUploadUrl,
    getDataServicesUrl : getDataServicesUrl,
    //Utility
